...
find "${VAGRANT_CORE_FOLDER}/files/exec-always" -maxdepth 1 -type f -exec chmod +x '{}' \; -exec {} \;
...
if [[ ! -f /.puphpet-stuff/exec-once-ran && (${#files[@]} -gt 0) ]]; then
    ...
    find "${VAGRANT_CORE_FOLDER}/files/exec-once" -maxdepth 1 -type f -exec chmod +x '{}' \; -exec {} \;
